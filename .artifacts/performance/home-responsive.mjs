...
await page.screenshot({path:'.artifacts/performance/home-'+locale+'-'+width+'.png',fullPage:true});results.push({locale,width,early,final,errors});await page.close();
}await browser.close();await fs.writeFile('.artifacts/performance/home-responsive.json',JSON.stringify(results,null,2));console.log(results.map(r=>({locale:r.locale,width:r.width,cls:r.final.cls,overflow:r.final.overflow,stable:JSON.stringify(r.early.hero)===JSON.stringify(r.final.hero),errors:r.errors})));
